...
# load pkgs
packages <- c("dplyr", "tidyverse", "readxl", "openxlsx", "ggplot2", "gtools", "kableExtra", "patchwork")
...
prop_missing_pervar <- as.data.frame(cbind(
  "# LLOQ" = sort(colSums(dt_lloqcheck == "LLOQ")),
  "% LLOQ" = round(sort(colMeans(dt_lloqcheck == "LLOQ")) * 100, 2),
  "# nd" = sort(colSums(dt_lloqcheck == "n.d.")),
  "% nd" = round(sort(colMeans(dt_lloqcheck == "n.d.")) * 100, 2)
))
...
prop_missing_pervar <- as.data.frame(cbind(
  "# LLOQ-BIB" = sort(colSums(dt_lloqcheck_cohort[[1]] == "LLOQ")),
  "% LLOQ-BIB" = round(sort(colMeans(dt_lloqcheck_cohort[[1]] == "LLOQ")) * 100, 2),
  "# LLOQ-EDEN" = sort(colSums(dt_lloqcheck_cohort[[2]] == "LLOQ")),
  "% LLOQ-EDEN" = round(sort(colMeans(dt_lloqcheck_cohort[[2]] == "LLOQ")) * 100, 2),
  "# LLOQ-KAN" = sort(colSums(dt_lloqcheck_cohort[[3]] == "LLOQ")),
  "% LLOQ-KAN" = round(sort(colMeans(dt_lloqcheck_cohort[[3]] == "LLOQ")) * 100, 2),
  "# LLOQ-MOBA" = sort(colSums(dt_lloqcheck_cohort[[4]] == "LLOQ")),
  "% LLOQ-MOBA" = round(sort(colMeans(dt_lloqcheck_cohort[[4]] == "LLOQ")) * 100, 2),
  "# LLOQ-RHEA" = sort(colSums(dt_lloqcheck_cohort[[5]] == "LLOQ")),
  "% LLOQ-RHEA" = round(sort(colMeans(dt_lloqcheck_cohort[[5]] == "LLOQ")) * 100, 2),
  "# LLOQ-SAB" = sort(colSums(dt_lloqcheck_cohort[[6]] == "LLOQ")),
  "% LLOQ-SAB" = round(sort(colMeans(dt_lloqcheck_cohort[[6]] == "LLOQ")) * 100, 2)
))
...
mean_sex <- df_full %>%
  filter(!is.na(h_sex)) %>%
  select(cortisol_prod_log10, h_sex) %>%
  group_by(h_sex) %>%
  summarise_all(mean, na.rm = TRUE)

df_p <- df_full %>% filter(!is.na(h_sex))

p_sex <- ggplot(df_p, aes(x = cortisol_prod_log10, fill = h_sex, color = h_sex)) +
  # geom_density(alpha=0.3)+
  geom_histogram(position = "identity", alpha = 0.5) +
  geom_vline(
    data = mean_sex, aes(xintercept = cortisol_prod_log10, color = h_sex),
    linetype = "dashed"
  ) +
  ggtitle("By sex")
