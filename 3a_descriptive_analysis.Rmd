...
cov <- c("h_sex", "hs_mvpa", "hs_sd_wk", # child characteristics
...
```{r cov, fig.width = 25 , fig.height = 25}
df_plot <- df_final %>%
  select(
...
p2 <- df_final %>%
  ggplot(aes(x = h_sex, fill = child_age_grp)) +
  geom_bar(position = "dodge") +
...
cardio <- df_final %>%
  select(h_sex, hs_zsys_bp, hs_zdia_bp, bp_pulse_avg)
...
anthro <- df_final %>%
  select(h_sex, hs_zbmi_who, hs_zwaist_mets, hs_skf_sum2, hs_freefatmass_bia, hs_fatmass_bia, hs_fatprop_bia)
anthro %>%
...
metab <- df_final %>%
  select(
    h_sex, hs_hdlchol_c, hs_ldlchol_c, hs_triglyc_c, hs_totchol_c, Leptin, Adiponectin,
    Cpeptide, INSULIN, Glucose, hs_creatinine_c
  )
metab %>%
...
cort <- df_final %>%
  select(h_sex, cortisol_prod_log10)
cort %>%
...
knitr::purl("descriptive_analysis.Rmd", documentation = 1L)
```
