---
title: "Plots"
author: "Yuchan Mou"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
    toc_float: true
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE, message=FALSE}
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE)
options(knitr.table.format = "html")
```

```{r load packages, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
# load pkgs
packages <- c("tidyverse", "labelled", "openxlsx", "meta", "readxl", "RColorBrewer", "ggsci")
sapply(packages, library, character.only = T)
```

```{r import data, eval=TRUE}
analysis_date <- Sys.Date()

# Air pollutants
# Count-based AL, Continuous AL and their weighted average
ap_tot_out <- read_excel("/Users/ymou/helix_project/results/airpollution_total_mainoutcome2023-10-14.xlsx")
# Count-based AL and weighted average score per system
ap_tot_outps <- read_excel("/Users/ymou/helix_project/results/airpollution_total_outcomepsystem2023-10-14.xlsx")
# Continuous AL and weighted average score per system
ap_tot_outzps <- read_excel("/Users/ymou/helix_project/results/airpollution_total_outcomepsystem_zscore2023-10-14.xlsx")

# Count-based AL, Continuous AL and their weighted average: Home, school, commuting
ap_hsr_out <- read_excel("/Users/ymou/helix_project/results/airpollution_hsrp_mainoutcome2023-10-14.xlsx") %>% filter(!str_detect(Predictor, "_p$")) # other places not incl
# Count-based AL and weighted average score per system: Home, school, commuting
ap_hsr_outps <- read_excel("/Users/ymou/helix_project/results/airpollution_hsrp_outcomepsystem2023-10-14.xlsx") %>% filter(!str_detect(Predictor, "_p$")) # other places not incl
# Continuous AL and weighted average score per system: Home, school, commuting
ap_hsr_outzps <- read_excel("/Users/ymou/helix_project/results/airpollution_hsrp_outcomepsystem_zscore2023-10-14.xlsx") %>% filter(!str_detect(Predictor, "_p$")) # other places not incl

# Noise
noise_tot_out <- read_excel("/Users/ymou/helix_project/results/noise_mainoutcome2023-10-14.xlsx")
```

```{r merge data}
ap_tot_res <- bind_rows(ap_tot_out, ap_tot_outps, ap_tot_outzps)
ap_tot_res <- ap_tot_res %>%
  mutate(al_type = if_else(str_detect(Outcome, "^AL_2") == T & str_detect(Outcome, "_wavg$") == F, "AL_2",
    if_else(str_detect(Outcome, "^AL_2") == T & str_detect(Outcome, "_wavg$") == T, "AL_2_wavg",
      if_else(str_detect(Outcome, "^AL_z2") == T & str_detect(Outcome, "_wavg$") == F, "AL_z2",
        if_else(str_detect(Outcome, "^AL_z2") == T & str_detect(Outcome, "_wavg$") == T, "AL_z2_wavg", NA)
      )
    )
  )) %>%
  dplyr::rename(
    "beta" = "beta_mod4",
    "low" = "cilo_mod4",
    "hi" = "cihi_mod4"
  ) %>%
  mutate_at(c("Predictor", "Outcome"), as.factor) %>% 
  mutate(Predictor = fct_relevel(Predictor,c("hs_no2_yr_hs_t","hs_pm25_yr_hs_t","hs_pm10_yr_hs_t", "hs_pm25abs_yr_hs_t")),
         Outcome = fct_relevel(Outcome, c("AL_2_tot", "AL_2_cardio", "AL_2_metab", "AL_2_immune", "AL_2_neuro",
                                          "AL_z2_tot", "AL_z2_cardio", "AL_z2_metab", "AL_z2_immune", "AL_z2_neuro",
                                          "AL_2_tot_wavg", "AL_2_cardio_wavg", "AL_2_metab_wavg", "AL_2_immune_wavg", "AL_2_neuro_wavg",
                                          "AL_z2_tot_wavg", "AL_z2_cardio_wavg", "AL_z2_metab_wavg", "AL_z2_immune_wavg", "AL_z2_neuro_wavg"))) %>% 
  mutate(
    Predictor = recode(Predictor,
      "hs_no2_yr_hs_t" = "NO[2]",
      "hs_pm10_yr_hs_t" = "PM[10]",
      "hs_pm25_yr_hs_t" = "PM[2.5]",
      "hs_pm25abs_yr_hs_t" = "PM[2.5][abs]"
    ),
    Outcome = recode(Outcome,
      "AL_2_tot" = "Count-based AL score",
      "AL_2_tot_wavg" = "Count-based weighted average AL score",
      "AL_z2_tot" = "Continuous AL score",
      "AL_z2_tot_wavg" = "Continuous weighted average AL score",
      "AL_2_cardio" = "Count-based cardiovascular AL",
      "AL_2_metab" = "Count-based metabolic AL",
      "AL_2_immune" = "Count-based immune/imflammatory AL",
      "AL_2_neuro" = "Count-based neuroendorine AL",
      "AL_2_cardio_wavg" = "Count-based cardiovascular weighted average AL",
      "AL_2_metab_wavg" = "Count-based metabolic weighted average AL",
      "AL_2_immune_wavg" = "Count-based immune/imflammatory weighted average AL",
      "AL_2_neuro_wavg" = "Count-based neuroendorine weighted average AL",
      "AL_z2_cardio" = "Continuous cardiovascular AL",
      "AL_z2_metab" = "Continuous metabolic AL",
      "AL_z2_immune" = "Continuous immune/imflammatory AL",
      "AL_z2_neuro" = "Continuous neuroendorine AL",
      "AL_z2_cardio_wavg" = "Continuous cardiovascular weighted average AL",
      "AL_z2_metab_wavg" = "Continuous metabolic weighted average AL",
      "AL_z2_immune_wavg" = "Continuous immune/imflammatory weighted average AL",
      "AL_z2_neuro_wavg" = "Continuous neuroendorine weighted average AL"
    )
  ) 



ap_hsr_res <- bind_rows(ap_hsr_out, ap_hsr_outps, ap_hsr_outzps)
ap_hsr_res <- ap_hsr_res %>%
  mutate(al_type = if_else(str_detect(Outcome, "^AL_2") == T & str_detect(Outcome, "_wavg$") == F, "AL_2",
    if_else(str_detect(Outcome, "^AL_2") == T & str_detect(Outcome, "_wavg$") == T, "AL_2_wavg",
      if_else(str_detect(Outcome, "^AL_z2") == T & str_detect(Outcome, "_wavg$") == F, "AL_z2",
        if_else(str_detect(Outcome, "^AL_z2") == T & str_detect(Outcome, "_wavg$") == T, "AL_z2_wavg", NA)
      )
    )
  )) %>%
  dplyr::rename(
    "beta" = "beta_mod4",
    "low" = "cilo_mod4",
    "hi" = "cihi_mod4"
  ) %>%
  mutate(type = if_else(str_detect(Predictor, "_h$"), "Home",
    if_else(str_detect(Predictor, "_s$"), "School", "Commuting")
  )) %>%
  mutate_at(c("Predictor", "Outcome"), as.factor) %>% 
  mutate(Predictor = fct_relevel(Predictor,c("hs_no2_yr_hs_h","hs_no2_yr_hs_s","hs_no2_yr_hs_r",
                                             "hs_pm25_yr_hs_h", "hs_pm25_yr_hs_s", "hs_pm25_yr_hs_r",
                                             "hs_pm10_yr_hs_h", "hs_pm10_yr_hs_s", "hs_pm10_yr_hs_r",
                                             "hs_pm25abs_yr_hs_h", "hs_pm25abs_yr_hs_s", "hs_pm25abs_yr_hs_r")),
         Outcome = fct_relevel(Outcome, c("AL_2_tot", "AL_2_cardio", "AL_2_metab", "AL_2_immune", "AL_2_neuro",
                                          "AL_z2_tot", "AL_z2_cardio", "AL_z2_metab", "AL_z2_immune", "AL_z2_neuro",
                                          "AL_2_tot_wavg", "AL_2_cardio_wavg", "AL_2_metab_wavg", "AL_2_immune_wavg", "AL_2_neuro_wavg",
                                          "AL_z2_tot_wavg", "AL_z2_cardio_wavg", "AL_z2_metab_wavg", "AL_z2_immune_wavg", "AL_z2_neuro_wavg"))) %>% 
  mutate(Predictor = recode(Predictor,
    "hs_no2_yr_hs_h" = "NO[2]",
    "hs_no2_yr_hs_s" = "NO[2]",
    "hs_no2_yr_hs_r" = "NO[2]",
    "hs_pm10_yr_hs_h" = "PM[10]",
    "hs_pm10_yr_hs_s" = "PM[10]",
    "hs_pm10_yr_hs_r" = "PM[10]",
    "hs_pm25_yr_hs_h" = "PM[2.5]",
    "hs_pm25_yr_hs_s" = "PM[2.5]",
    "hs_pm25_yr_hs_r" = "PM[2.5]",
    "hs_pm25abs_yr_hs_h" = "PM[2.5][abs]",
    "hs_pm25abs_yr_hs_s" = "PM[2.5][abs]",
    "hs_pm25abs_yr_hs_r" = "PM[2.5][abs]"
  ),
    Outcome = recode(Outcome,
      "AL_2_tot" = "Count-based AL score",
      "AL_2_tot_wavg" = "Count-based weighted average AL score",
      "AL_z2_tot" = "Continuous AL score",
      "AL_z2_tot_wavg" = "Continuous weighted average AL score",
      "AL_2_cardio" = "Count-based cardiovascular AL",
      "AL_2_metab" = "Count-based metabolic AL",
      "AL_2_immune" = "Count-based immune/imflammatory AL",
      "AL_2_neuro" = "Count-based neuroendorine AL",
      "AL_2_cardio_wavg" = "Count-based cardiovascular weighted average AL",
      "AL_2_metab_wavg" = "Count-based metabolic weighted average AL",
      "AL_2_immune_wavg" = "Count-based immune/imflammatory weighted average AL",
      "AL_2_neuro_wavg" = "Count-based neuroendorine weighted average AL",
      "AL_z2_cardio" = "Continuous cardiovascular AL",
      "AL_z2_metab" = "Continuous metabolic AL",
      "AL_z2_immune" = "Continuous immune/imflammatory AL",
      "AL_z2_neuro" = "Continuous neuroendorine AL",
      "AL_z2_cardio_wavg" = "Continuous cardiovascular weighted average AL",
      "AL_z2_metab_wavg" = "Continuous metabolic weighted average AL",
      "AL_z2_immune_wavg" = "Continuous immune/imflammatory weighted average AL",
      "AL_z2_neuro_wavg" = "Continuous neuroendorine weighted average AL"
    ))


noise_tot_out <- noise_tot_out %>%
  mutate(al_type = if_else(str_detect(Outcome, "^AL_2") == T & str_detect(Outcome, "_wavg$") == F, "AL_2",
    if_else(str_detect(Outcome, "^AL_2") == T & str_detect(Outcome, "_wavg$") == T, "AL_2_wavg",
      if_else(str_detect(Outcome, "^AL_z2") == T & str_detect(Outcome, "_wavg$") == F, "AL_z2",
        if_else(str_detect(Outcome, "^AL_z2") == T & str_detect(Outcome, "_wavg$") == T, "AL_z2_wavg", NA)
      )
    )
  )) %>%
  mutate(pred_type = ifelse(str_detect(Predictor, "^hs_lden_c_h") == T, "lden_h", "lden_s")) %>%
  dplyr::rename(
    "beta" = "beta_mod4",
    "low" = "cilo_mod4",
    "hi" = "cihi_mod4"
  ) %>%
  mutate_at(c("Predictor", "pred_type", "Outcome"), as.factor) %>%
  mutate(Predictor = fct_relevel(Predictor,c("hs_lden_c_h_55-59.9","hs_lden_c_h_60-64.9","hs_lden_c_h>=65",
                                             "hs_lden_c_s_55-59.9", "hs_lden_c_s_60-64.9", "hs_lden_c_s>=65")),
         Outcome = fct_relevel(Outcome, c("AL_2_tot", "AL_z2_tot", "AL_2_tot_wavg", "AL_z2_tot_wavg"))) %>% 
  mutate(
    Predictor = recode(Predictor,
      "hs_lden_c_h_55-59.9" = "55-59.9",
      "hs_lden_c_h_60-64.9" = "60-64.9",
      "hs_lden_c_h>=65" = ">=65",
      "hs_lden_c_s_55-59.9" = "55-59.9",
      "hs_lden_c_s_60-64.9" = "60-64.9",
      "hs_lden_c_s>=65" = ">=65"
    ),
    pred_type = recode(pred_type,
      "lden_h" = "Road traffic noise at home address",
      "lden_s" = "Road traffic noise at school address"
    ),
    Outcome = recode(Outcome,
      "AL_2_tot" = "Count-based AL score",
      "AL_2_tot_wavg" = "Count-based weighted average AL score",
      "AL_z2_tot" = "Continuous AL score",
      "AL_z2_tot_wavg" = "Continuous weighted average AL score"
    )
  )
```

# Plots

## Air pollutants

### Count-based AL score
```{r}
dt_p1 <- ap_tot_res %>% filter(al_type == "AL_2")
```

```{r fig.asp = 0.8, fig.width=10}
p1 <- ggplot(
  data = dt_p1,
  aes(x = Outcome, y = exp_beta_mod4, ymin = exp_cilo_mod4, ymax = exp_cihi_mod4)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 1, linetype = 2) +
  xlab("") +
  ylab("Relative risk (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = exp_cilo_mod4, ymax = exp_cihi_mod4, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~Predictor, strip.position = "left", nrow = 9, scales = "free_y", labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y.left = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p1
```

```{r, eval=F}
ggsave("/Users/ymou/helix_project/results/Fig/fig-countAL.png", width = 10, height = 10, units = "in", dpi = 300)
```


### Count-based weighted average AL score
```{r}
dt_p2 <- ap_tot_res %>% filter(al_type == "AL_2_wavg")
```

```{r fig.asp = 0.8, fig.width=10}
p2 <- ggplot(
  data = dt_p2,
  aes(x = Outcome, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~Predictor, strip.position = "left", nrow = 9, scales = "free_y", labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y.left = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p2
```


### Continuous AL score
```{r}
dt_p3 <- ap_tot_res %>% filter(al_type == "AL_z2")
```

```{r fig.asp = 0.8, fig.width=10}
p3 <- ggplot(
  data = dt_p3,
  aes(x = Outcome, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~Predictor, strip.position = "left", nrow = 9, scales = "free_y", labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y.left = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p3
```

```{r, eval=F}
ggsave("/Users/ymou/helix_project/results/Fig/fig-continuousAL.png", width = 10, height = 10, units = "in", dpi = 300)
```


### Continuous weighted average AL score
```{r}
dt_p4 <- ap_tot_res %>% filter(al_type == "AL_z2_wavg")
```

```{r fig.asp = 0.8, fig.width=10}
p4 <- ggplot(
  data = dt_p4,
  aes(x = Outcome, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~Predictor, strip.position = "left", nrow = 9, scales = "free_y", labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y.left = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p4
```

## Air pollutants(home, school, commuting) 

### Count-based AL score
```{r}
dt_p5 <- ap_hsr_res %>% filter(al_type == "AL_2")
```

```{r fig.asp = 0.8, fig.width=10}
p5 <- ggplot(
  data = dt_p5,
  aes(x = Outcome, y = exp_beta_mod4, ymin = exp_cilo_mod4, ymax = exp_cihi_mod4)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 1, linetype = 2) +
  xlab("") +
  ylab("Relative risk (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = exp_cilo_mod4, ymax = exp_cihi_mod4, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_grid(Predictor ~ type, labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p5
```


```{r, eval=F}
ggsave("/Users/ymou/helix_project/results/Fig/fig-countAL-hsr.png", width = 10, height = 10, units = "in", dpi = 300)
```


### Count-based weighted average AL score
```{r}
dt_p6 <- ap_hsr_res %>% filter(al_type == "AL_2_wavg")
```

```{r fig.asp = 1, fig.width=10}
p6 <- ggplot(
  data = dt_p6,
  aes(x = Outcome, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_grid(Predictor ~ type, labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p6
```

### Continuous AL score
```{r}
dt_p7 <- ap_hsr_res %>% filter(al_type == "AL_z2")
```

```{r fig.asp = 0.8, fig.width=10}
p7 <- ggplot(
  data = dt_p7,
  aes(x = Outcome, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_grid(Predictor ~ type, labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p7
```

```{r, eval=F}
ggsave("/Users/ymou/helix_project/results/Fig/fig-continousAL-hsr.png", width = 10, height = 10, units = "in", dpi = 300)
```

### Continuous weighted average AL score
```{r}
dt_p8 <- ap_hsr_res %>% filter(al_type == "AL_z2_wavg")
```

```{r fig.asp = 1, fig.width=10}
p8 <- ggplot(
  data = dt_p8,
  aes(x = Outcome, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Outcome)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Outcome), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_grid(Predictor ~ type, labeller = labeller(Predictor = label_parsed)) +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p8
```


## Road traffic noise
### Count-based AL score
```{r}
dt_p9 <- noise_tot_out %>% filter(al_type == "AL_2")
```

```{r fig.asp = 0.8, fig.width=10}
p9 <- ggplot(
  data = dt_p9,
  aes(x = Predictor, y = exp_beta_mod4, ymin = exp_cilo_mod4, ymax = exp_cihi_mod4)
) +
  geom_pointrange(aes(col = Predictor)) +
  geom_hline(yintercept = 1, linetype = 2) +
  xlab("") +
  ylab("Relative risk (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = exp_cilo_mod4, ymax = exp_cihi_mod4, col = Predictor), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~pred_type, strip.position = "left", nrow = 9, scales = "free_y") +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y.left = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p9
```

```{r, eval=F}
ggsave("/Users/ymou/helix_project/results/Fig/fig-countAL-noise.png", width = 10, height = 10, units = "in", dpi = 300)
```

### Continuous AL score
```{r}
dt_p10 <- noise_tot_out %>% filter(al_type == "AL_z2")
```

```{r fig.asp = 0.8, fig.width=10}
p10 <- ggplot(
  data = dt_p10,
  aes(x = Predictor, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = Predictor)) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = Predictor), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~pred_type, strip.position = "left", nrow = 9, scales = "free_y") +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    axis.text.x = element_text(face = "bold"),
    axis.title = element_text(size = 12, face = "bold"),
    strip.text.y.left = element_text(angle = 0, face = "bold")
  ) +
  coord_flip() +
  ggtitle("")

p10
```

```{r, eval=F}
ggsave("/Users/ymou/helix_project/results/Fig/fig-continuousAL-noise.png", width = 10, height = 10, units = "in", dpi = 300)
```
