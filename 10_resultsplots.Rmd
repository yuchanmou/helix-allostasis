...
noise_tot_out <- noise_tot_out %>%
  ...
  rename(`Categories of noise levels (dB)` = Predictor)
```
...
p1 <- ggplot(
  ...
  geom_pointrange(aes(col = Outcomes)) +
...
p9 <- ggplot(
  data = dt_p9,
  aes(x = `Categories of noise levels (dB)`, y = exp_beta_mod4, ymin = exp_cilo_mod4, ymax = exp_cihi_mod4)
) +
  geom_pointrange(aes(col = `Categories of noise levels (dB)`)) +
  geom_hline(yintercept = 1, linetype = 3) +
  xlab("") +
  ylab("Relative risk (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = exp_cilo_mod4, ymax = exp_cihi_mod4, col = `Categories of noise levels (dB)`), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
...
p10 <- ggplot(
  data = dt_p10,
  aes(x = `Categories of noise levels (dB)`, y = beta, ymin = low, ymax = hi)
) +
  geom_pointrange(aes(col = `Categories of noise levels (dB)`)) +
  geom_hline(yintercept = 0, linetype = 3) +
  xlab("") +
  ylab("Effect sizes (95% Confidence Interval)") +
  geom_errorbar(aes(ymin = low, ymax = hi, col = `Categories of noise levels (dB)`), width = 0.5, cex = 1) +
  scale_color_npg() +
  guides(color = guide_legend(reverse = TRUE)) +
  facet_wrap(~pred_type, strip.position = "left", nrow = 9, scales = "free_y") +
  # scale_y_continuous(limits = c(-1, 6), breaks = c(-1, 0, 1, 2, 3, 4, 5, 6)) +
